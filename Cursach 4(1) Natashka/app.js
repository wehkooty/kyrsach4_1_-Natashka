(function(){
  ...
  function nextId(items){ return (items.reduce((m,x)=>Math.max(m,x.id),0) || 0) + 1; }
  ...
  window.exportStatisticsToExcel = function() {
    try {
      ...
      const excelData = [
        ...
        { 'Метрика': 'Клуб с наибольшим доходом', 'Значение': stats.highestEarningClub }
      ];
  ...
  function generateApplicationStatistics() {
    ...
    const mostActiveClub = clubEventCounts.reduce((max, current) => 
      current.eventCount > max.eventCount ? current : max, clubEventCounts[0])?.club?.name || 'Нет данных';
    
    // Находим клуб с наибольшим доходом
    const clubIncomes = clubs.map(club => ({
      club,
      income: payments.filter(p => p.clubId === club.id).reduce((sum, p) => sum + (p.amount || 0), 0)
    }));
    const highestEarningClub = clubIncomes.reduce((max, current) => 
      current.income > max.income ? current : max, clubIncomes[0])?.club?.name || 'Нет данных';
  ...
  function Clubs(){
    ...
    app().innerHTML = h`
      ...
      <table class="table" id="clubsTable">
        <thead><tr><th>${t.name}</th><th>${t.description}</th><th>${t.owner}</th><th>${t.balance}</th><th>${t.actions}</th></tr></thead>
        <tbody>
          ${clubs.map(c=>h`<tr>
            <td>${c.name}</td>
            <td>${c.description||''}</td>
            <td>${db.users.find(u=>u.id===c.ownerId)?.name||'?'} </td>
            <td>${formatCurrency(calculateClubBalance(c.id))}</td>
            <td class="actions">
    ...
    function filterClubs() {
      ...
      rows.forEach(row => {
        ...
        const ownerId = row.querySelector('[data-del]')?.getAttribute('data-del');
    ...
    app().querySelectorAll('[data-del]').forEach(b=>b.onclick=()=>{
      ...
      if (confirm(t.confirmDelete)) {
        ...
        const evs = db.events.filter(e=>e.clubId!==id); 
        const evIds = new Set(evs.map(e=>e.id));
        db.attendance = db.attendance.filter(a=>!evIds.has(a.eventId)); 
        db.events = evs; 
  ...
  function Events(clubId){
    ...
    app().innerHTML = h`
      ...
      <table class="table" id="eventsTable">
        <thead><tr><th>Название</th><th>${t.starts}</th><th>${t.ends}</th><th>${t.actions}</th></tr></thead>
        <tbody>
          ${events.map(ev=>h`<tr>
            <td>${ev.title}</td>
    ...
    function filterEvents() {
      ...
      rows.forEach(row => {
        ...
        const startsAt = row.cells[1].textContent;
    ...
    app().innerHTML = h`
      ...
      <div class="card main-card">
        ...
        <div class="action-group primary">
          ${isReg ? h`<button class="btn danger" id="unreg">${t.unregister}</button>` : h`<button class="btn" id="reg">${t.register}</button>`}
        </div>
      </div>
    `;
  ...
  function EventEdit(clubId, eventId){
    const user = me(); if(!user){ go('/login'); return; }
    const club = db.clubs.find(c=>c.id===clubId); const ev = db.events.find(e=>e.id===eventId && e.clubId===clubId);
    if(!club||!ev||(club.ownerId!==user.id && user.role!==Roles.ADMIN)){ go(`/clubs/${clubId}/events/${eventId}`); return; }
      ...
      <div class="card main-card">
        ...
        <div class="row">
          <div class="col">
            ...
            <p><strong>Клуб с наибольшим доходом:</strong> ${stats.highestEarningClub}</p>
          </div>
        </div>
  ...
  const routes = {
    ...
    '/clubs/:id/members': ClubMembers, '/clubs/:id/finances': ClubFinances, '/clubs/:id/schedule': ClubSchedule,
  ...
  function dispatch(){
    ...
    if (p[0]==='clubs' && p.length===2 && p[1]==='edit') return routes['/clubs']();
